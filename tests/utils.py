...
def get_pytorch_model(model, use_cache=True):
    def forward(
        inputs: torch.Tensor, past_key_values=None
    ) -> (torch.Tensor, Optional[List[torch.FloatTensor]]):
...
def get_tokenizer(model_path):
    if "cohere-gpt" in model_path:
        return AutoTokenizer.from_pretrained("gpt2-xl")
