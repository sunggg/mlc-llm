...
def test_stop(
    ...
    stop = ["\n", "!"]
    requests = [create_request(idx=str(0), prompt=prompt, temp=0, max_tokens=200, stop=stop, ignore_eos=True)]
    engine.add(requests)
    ...
    while engine.has_pending_requests():
        ...
        for res in results.outputs:
            ...
            if seq.is_finished:
                print(f"generated: {generated[int(res.request_id)]!r}")
                #assert seq.num_generated_tokens == requests[int(res.request_id)].stopping_criteria.max_tokens
                #assert seq.finish_reason == FinishReason.Length
            else:
                generated[int(res.request_id)] += seq.delta
...
if __name__ == "__main__":
    ...
    parser.add_argument("--local-id", type=str, default="llama-2-7b-chat-hf-q0f16-presharded-1gpu")
    #required=True)
